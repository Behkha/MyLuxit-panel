<template>
  <v-container>
    <h2 style="text-align: right;margin-bottom: 25px;margin-top: 10px;">لیست عکاسان</h2>
    <v-layout>
      <v-flex xs12>
        <div>
          <v-card>
...
<script>
  ...
  export default {
    data: function () {
      return {
        ...
        headers: [
          ...
          {text: 'نام عکاس', value: 'name', align: 'right', sortable: false},
    ...
    methods: {
      getDataFromApi () {
        this.loading = true
        this.$store.dispatch('IndexPhotographers', {
      ...
      operation (id) {
        this.$router.push({name: 'PhotographerEdit', params: {id: id}})
      },
      removeSort () {
