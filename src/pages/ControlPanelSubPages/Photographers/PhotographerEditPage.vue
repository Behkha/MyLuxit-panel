<template xmlns:v-slot="http://www.w3.org/1999/XSL/Transform">
  <v-container>
    <h2 style="text-align: right;margin-bottom: 25px;margin-top: 10px;">ویرایش عکاس</h2>
      ...
      <v-layout >
        <v-dialog v-model="dialog" persistent width="500">
          <template v-slot:activator="{ on }">
            <v-btn v-on="on" outline large color="red white-1 white--text">حذف عکاس
            </v-btn>
          </template>
          <v-card>
            <v-card-title>تایید عملیات حذف</v-card-title>

            <v-card-text>آیا واقعا میخواهید این عکاس را حذف کنید ؟</v-card-text>
            ...
            <v-card-actions>
              ...
              <v-btn
                ...
                flat="flat"
                @click="deletePhotographer($route.params.id) ">
                بله
              </v-btn>
  ...
  export default {
    ...
    methods: {
      submitForm () {
        ...
        this.$store.dispatch('editPhotographerRequest', form)
          .then(response => {
            this.form.submitButtonLoading = false
            this.form.submitted = true
            this.clearForm()
            setTimeout(() => {
              this.$router.push({name: 'PhotographersList'})
            }, 500)
          })
          .catch(error => {
            this.form.submitButtonLoading = false
      ...
      deletePhotographer (id) {
        this.$store.dispatch('deletePhotographerRequest', id)
          .then(response => {
            this.form.submitButtonLoading = false
            this.form.submitted = true
            this.clearForm()
      ...
      initialForm () {
        this.$store.dispatch('showPhotographer', this.$route.params.id)
          .then(response => {
