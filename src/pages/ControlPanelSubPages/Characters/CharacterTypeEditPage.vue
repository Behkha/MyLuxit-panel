<template xmlns:v-slot="http://www.w3.org/1999/XSL/Transform">
  <v-container>
    <h2 style="text-align: right;margin-bottom: 25px;margin-top: 10px;">ویرایش نوع شخصیت</h2>
    ...
    <form>
      ...
      <v-layout   class="wrap">
        <v-flex class="xs4 offset-xs1">
          <v-text-field label="نام" v-model="form.fields.title"></v-text-field>
        </v-flex>
      </v-layout>
      ...
      <v-layout >
        ...
        <v-dialog v-model="dialog" persistent width="500">
          <template v-slot:activator="{ on }">
            <v-btn v-on="on" outline large color="red white-1 white--text">حذف نوع شخصیت
            </v-btn>
          </template>
          <v-card>
            <v-card-title>تایید عملیات حذف</v-card-title>

            <v-card-text>آیا واقعا میخواهید این نوع شخصیت را حذف کنید ؟</v-card-text>
            ...
            <v-card-actions>
              ...
              <v-btn
                ...
                flat="flat"
                @click="deleteCharacterType($route.params.id) ">
                بله
              </v-btn>
...
<script>
  export default {
    ...
    methods: {
      submitForm () {
        let form = {
          title: this.form.fields.title,
          id: this.$route.params.id
        }
        this.form.submitButtonLoading = true
        this.$store.dispatch('editCharacterTypeRequest', form)
          .then(response => {
            this.form.submitButtonLoading = false
            this.form.submitted = true
            this.clearForm()
            setTimeout(() => {
              this.$router.push({name: 'CharacterTypesList'})
            }, 500)
          })
          .catch(error => {
            this.form.submitButtonLoading = false
      ...
      deleteCharacterType (id) {
        this.$store.dispatch('deleteCharacterTypeRequest', id)
          .then(response => {
            this.form.submitButtonLoading = false
            this.form.submitted = true
            this.clearForm()
      ...
      initialForm () {
        this.$store.dispatch('showCharacterType', this.$route.params.id)
          .then(response => {
            this.form.fields.title = response.data.title
            // this.initialForm()
          })
