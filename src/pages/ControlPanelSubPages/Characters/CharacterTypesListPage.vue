<template>
  <v-container>
    <h2 style="text-align: right;margin-bottom: 25px;margin-top: 10px;">لیست انواع شخصیت</h2>
    <v-layout>
      <v-flex xs12>
        <div>
          <v-card>
  ...
  export default {
    data: function () {
      return {
        ...
        headers: [
          {text: 'ID', value: 'title', align: 'right', sortable: false},
          {text: 'نام نوع شخصیت', value: 'title', align: 'right', sortable: false},
    ...
    methods: {
      getDataFromApi () {
        this.loading = true
        this.$store.dispatch('IndexCharacterTypes', {
      ...
      operation (id) {
        this.$router.push({name: 'CharacterTypeEdit', params: {id: id}})
      },
      removeSort () {
