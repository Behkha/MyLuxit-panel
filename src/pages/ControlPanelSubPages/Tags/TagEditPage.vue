...
<v-container>
  <h2 style="text-align: right;margin-bottom: 25px;margin-top: 10px;">ویرایش تگ</h2>
    ...
    <v-layout>
      ...
      <v-dialog v-model="dialog" persistent width="500">
        <template v-slot:activator="{ on }">
          <!-- <v-btn v-on="on" outline large color="red white-1 white--text">حذف تگ
          </v-btn> -->
        </template>
        <v-card>
          <v-card-title>تایید عملیات حذف</v-card-title>

          <v-card-text>آیا واقعا میخواهید این تگ را حذف کنید ؟</v-card-text>

          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="green darken-1" flat="flat" @click="deleteTag($route.params.id) ">
              بله
            </v-btn>
...
export default {
  ...
  methods: {
    ...
    submitForm() {
      ...
      this.$store.dispatch('editTagRequest', form)
        .then(response => {
          this.form.submitButtonLoading = false
          this.form.submitted = true
          this.clearForm()
    ...
    initialForm() {
      this.$store.dispatch('showTag', this.$route.params.id)
        .then(response => {
