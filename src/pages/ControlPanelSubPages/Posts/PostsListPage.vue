<template>
  <v-container>
    <h2 style="text-align: right;margin-bottom: 25px;margin-top: 10px;">لیست پست ها</h2>
    <v-layout>
      <v-flex xs12>
        <div>
          <v-card>
            ...
            >
              <template slot="items" slot-scope="props">
                ...
                <td class="text-xs-right" v-if="props.item.postable_type === 'event'">{{props.item.postable.title}}</td>
                <td class="text-xs-right" v-else-if="props.item.postable_type === 'place'">{{props.item.postable.name}}</td>
                <td class="text-xs-right">{{props.item.postable.visits.total}}</td>
...
<script>
  ...
  export default {
    data: function () {
      return {
        ...
        headers: [
          ...
          {text: 'عنوان پست', value: 'name', align: 'right', sortable: false},
          {text: 'بازدید', value: 'name', align: 'right', sortable: false},
    ...
    methods: {
      getDataFromApi () {
        this.loading = true
        this.$store.dispatch('IndexPosts', {
