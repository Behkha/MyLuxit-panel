...
<v-container>
  <h2 style="text-align: right;margin-bottom: 25px;margin-top: 10px;">ویرایش نوع مکان</h2>
    ...
    <v-layout>
      ...
      <v-dialog v-model="dialog" persistent width="500">
        <template v-slot:activator="{ on }">
          <v-btn v-on="on" outline large color="red white-1 white--text">حذف نوع مکان
          </v-btn>
        </template>
        <v-card>
          <v-card-title>تایید عملیات حذف</v-card-title>

          <v-card-text>آیا واقعا میخواهید این نوع مکان را حذف کنید ؟</v-card-text>

          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="green darken-1" flat="flat" @click="deletePlaceType($route.params.id) ">
              بله
            </v-btn>
...
export default {
  data: () => {
    return {
      dialog: false,
      dialog_message: 'با موفقیت انجام شد',
      form: {
        fields: {
          name: null
        },
  ...
  methods: {
    submitForm() {
      ...
      this.$store.dispatch('editPlaceTypeRequest', form)
        .then(response => {
    ...
    deletePlaceType(id) {
      this.$store.dispatch('deletePlaceTypeRequest', id)
        .then(response => {
          if (response.data.message == "there are event associated with this type") {
            this.dialog_message = 'مکانی با این نوع ساخته شده قابل حذف نیست !'
          }
          this.form.submitButtonLoading = false
          this.form.submitted = true
          this.clearForm()
    ...
    initialForm() {
      this.$store.dispatch('showPlaceType', this.$route.params.id)
        .then(response => {
