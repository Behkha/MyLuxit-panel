<template>
  <v-container>
    <h2 style="text-align: right;margin-bottom: 25px;margin-top: 10px;">افزودن نوع مکان جدید</h2>
  ...
  export default {
    ...
    methods: {
      submitForm () {
        ...
        this.$store.dispatch('createPlaceTypeRequest', form)
          .then(response => {
            this.form.submitButtonLoading = false
            this.form.submitted = true
            this.clearForm()
            setTimeout(() => {
              this.$router.push({name: 'PlaceTypesList'})
            }, 500)
          })
          .catch(error => {
            this.form.submitButtonLoading = false
