<template>
<v-container>
  <h2 style="text-align: right;margin-bottom: 25px;margin-top: 10px;">لیست مکان ها</h2>
  <v-layout>
    <v-flex xs12>
      <div>
        <v-card>
          <v-data-table :headers="headers" :items="items" :pagination.sync="pagination" :total-items="totalItems" :loading="loading" class="elevation-1" style="direction: rtl" :rows-per-page-items="[15]" rows-per-page-test="تعداد در هر صفحه"
            :disable-initial-sort="true">
            <template slot="items" slot-scope="props">
              <td class="text-xs-right">{{props.item.id}}</td>
              <td class="text-xs-right">{{props.item.name}}</td>
              <!--<td class="text-xs-right">{{placeTypeToText(props.item.type_id)}}</td>-->
...
export default {
  ...
  methods: {
    placeTypeToText(id) {
      var result = ''
      this.place_type_array.forEach(function(item) {
    ...
    getDataFromApi() {
      ...
      this.$store.dispatch('IndexPlaces', {
          ...
          filters: {
            draft: this.showDrafts.toString(),
            publish_limit: 'false',
            keyword: this.search
          }
        })
        .then(response => {
          ...
          this.$store.dispatch('IndexPlaceTypesQ')
            .then(response => {
              this.place_type_array = response.data
            })
        })
    },
    operation(id) {
