<template>
  <v-container>
    <h2 style="text-align: right;margin-bottom: 25px;margin-top: 10px;">لیست انواع مکانها</h2>
    <v-layout>
      <v-flex xs12>
        <div>
          <v-card>
...
<script>
  ...
  export default {
    data: function () {
      return {
        ...
        headers: [
          ...
          {text: 'نام نوع مکان', value: 'name', align: 'right', sortable: false},
    ...
    methods: {
      getDataFromApi () {
        this.loading = true
        this.$store.dispatch('IndexPlaceTypes', {
      ...
      operation (id) {
        this.$router.push({name: 'PlaceTypeEdit', params: {id: id}})
      },
      removeSort () {
