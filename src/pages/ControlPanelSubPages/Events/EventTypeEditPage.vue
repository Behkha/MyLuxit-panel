...
<v-container>
  <h2 style="text-align: right;margin-bottom: 25px;margin-top: 10px;">ویرایش نوع رویداد</h2>
    ...
    <v-layout>
      ...
      <v-dialog v-model="dialog" persistent width="500">
        <template v-slot:activator="{ on }">
          <v-btn v-on="on" outline large color="red white-1 white--text">حذف نوع رویداد
          </v-btn>
        </template>
        <v-card>
          <v-card-title>تایید عملیات حذف</v-card-title>

          <v-card-text>آیا واقعا میخواهید این نوع رویداد را حذف کنید ؟</v-card-text>

          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="green darken-1" flat="flat" @click="deleteEventType($route.params.id) ">
              بله
            </v-btn>
...
export default {
  ...
  methods: {
    submitForm() {
      ...
      this.$store.dispatch('editEventTypeRequest', form)
        .then(response => {
    ...
    deleteEventType(id) {
      this.$store.dispatch('deleteEventTypeRequest', id)
        .then(response => {
          if (response.data.message == "there are event associated with this type") {
            this.dialog_message = 'رویدادی با این نوع ساخته شده است قابل حذف نیست !'
          }
          this.form.submitButtonLoading = false
          this.form.submitted = true
          this.clearForm()
        ...
        .catch(error => {
          if (error.response.data.message == "there are event associated with this type") {
            this.dialog_message = 'رویدادی با این نوع ساخته شده است قابل حذف نیست !'
          }
    ...
    initialForm() {
      this.$store.dispatch('showEventType', this.$route.params.id)
        .then(response => {
