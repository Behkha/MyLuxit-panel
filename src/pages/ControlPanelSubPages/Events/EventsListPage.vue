<template>
  <v-container>
    <h2 style="text-align: right;margin-bottom: 25px;margin-top: 10px;">لیست رویدادها</h2>
    <v-layout>
      <v-flex xs12>
        <div>
          <v-card>
            ...
            >
              <template slot="items" slot-scope="props">
                ...
                <td class="text-xs-right" v-if="props.item.type">{{props.item.type.name}}</td>
                <td class="text-xs-right" v-else="props.item.type">نامشخص</td>
                <!--<td class="text-xs-right">{{eventTypeIdToText(props.item.type_id)}}</td>-->
                <!--<td class="text-xs-right">{{placeIdToText(props.item.place_id)}}</td>-->
                <td class="text-xs-center">
                  <v-btn outline @click="operation(props.item.id)" color="green white-1 white--text">ویرایش یا حذف
                  </v-btn>
                </td>
              </template>
            </v-data-table>
...
<script>
  ...
  export default {
    data: function () {
      return {
        ...
        headers: [
          ...
          {text: 'نام رویداد', value: 'name', align: 'right', sortable: false},
          {text: 'نوع رویداد', value: 'name', align: 'right', sortable: false},
          // {text: 'مکان رویداد', value: 'name', align: 'right', sortable: false},
    ...
    methods: {
      eventTypeIdToText (id) {
        var result = ''
        this.events_array.forEach(function (item) {
      ...
      placeIdToText (id) {
        var result = ''
        this.places_array.forEach(function (item) {
          if (parseInt(item.id) === id) {
            result = item.name
          }
        })
      ...
      getDataFromApi () {
        this.loading = true
        this.$store.dispatch('IndexEvents', {
          ...
          .then(response => {
            ...
            this.$store.dispatch('IndexEventTypesQ')
              .then(response => {
                this.events_array = response.data
              })
            this.$store.dispatch('IndexPlacesQ')
              .then(response => {
                this.places_array = response.data.data
              })
          })
      },
      operation (id) {
